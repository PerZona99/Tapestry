...
// A YouTube video ID is 11 characters of letters/numbers/-/_ and nothing else.
// Anything else (like a URL ending in .mp4) is treated as a direct video file.
const isYouTubeId = value => /^[A-Za-z0-9_-]{11}$/.test(value || '');
...
function FilmPlayer({ film, mode }) {
  ...
  if (source && isYouTubeId(source)) return <iframe className="film-player" src={`https://www.youtube-nocookie.com/embed/${source}?autoplay=${mode === 'trailer' ? 1 : 0}&rel=0&modestbranding=1`} title={`${film.title} ${label}`} allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />;
...
function Modal({ item, setItem, kind, index, setIndex }) {
  ...
  const move = (amount) =>
    setIndex((index + amount + artworks.length) % artworks.length);
  return (
    <div
      className="modal-backdrop"
      onMouseDown={(e) => e.target === e.currentTarget && setItem(null)}
    >
      <section
      ...
      >
        ...
        {isArt && (
          <img className="modal-art" src={item.image} alt={item.title} />
        )}
        {kind === "film" && (
          <FilmPlayer film={item} mode={item.view || "program"} />
        )}
...
function SectionTitle({ overline, title, text }) {
  return (
    <header className="section-heading reveal">
      ...
      <h2>{title}</h2>
      {text && <p>{text}</p>}
      <img src="/assets/divider-brown.png" alt="" />
    </header>
  );
}
...
function Hero({ active }) {
  return (
    <section className="hero" id="home">
      <img className="hero-curtain left" src="/assets/curtain-red.png" alt="" />
      <img
        className="hero-curtain right"
        src="/assets/curtain-red.png"
        alt=""
      />
  ...
  return (
    <section id="literature" className="paper-section">
      ...
      <div className="section-decor literature-decor" aria-hidden="true">
        ...
        <img className="decor-bookplate" src="/assets/ex-libris.png" alt="" />
      </div>
      ...
      <p className="result-count">
        {query || showAll
          ...
          : `Showing ${visiblePoems.length} of ${filtered.length} pieces`}{" "}
        in the cabinet
      </p>
      <div className="poem-grid">
        {visiblePoems.map((poem, i) => (
          <article
            className="poem-card reveal shown"
            style={{ "--n": i }}
            key={poem.id}
          >
            <div>
      ...
      {!query && !showAll && filtered.length > visiblePoems.length && (
        <div className="collection-action reveal shown">
          <button onClick={() => setShowAll(true)}>
            Read more poems <span>↓</span>
          </button>
        </div>
      )}
      {!query && showAll && (
...
function Prose({ onOpen }) {
  return (
    <section id="prose" className="prose-section">
      <div className="section-decor prose-decor" aria-hidden="true">
        <img className="decor-building" src="/assets/Building.png" alt="" />
        <img className="decor-bookplate" src="/assets/ex-libris.png" alt="" />
      </div>
      <SectionTitle
        overline="The Reading Room"
        title="Prose & Other Wanderings"
        text="Essays and stories for those inclined to stay awhile."
      />
...
function Arts({ onOpen }) {
  return (
    <section id="visual-arts" className="arts-section">
      <SectionTitle
      ...
      <div className="gallery">
        {artworks.map((art, i) => (
          <button
            className={`art-card art-${i + 1} reveal`}
            onClick={() => onOpen(art, i)}
            key={art.title}
          >
...
function FilmThumbnail({ film, onOpenTrailer }) {
  ...
  const startPreview = () => {
    if (preview.current) preview.current.play().catch(() => {});
  };
  const stopPreview = () => {
    if (preview.current) {
  ...
  return (
    ...
    >
      {film.thumbnail && <img src={film.thumbnail} alt="" />}
      {film.trailerUrl && (
        <video
          ref={preview}
          muted
      ...
      {!film.thumbnail && !film.trailerUrl && (
        <span className="thumbnail-placeholder">Thumbnail coming soon</span>
      )}
...
function Theatre({ onOpen }) {
  return (
    <section id="theatre" className="theatre-section">
      <img src="/assets/theatre-stage.png" className="stage" alt="" />
      <div className="theatre-content">
        <SectionTitle
          overline="The Midnight Matinée"
          title="Theatre"
          text="Two beloved rooms of the cinematic imagination."
        />
        <div className="film-group">
          {films.map((film) => (
            <article className="film-card reveal" key={film.title}>
              <FilmThumbnail
                film={film}
                onOpenTrailer={() => onOpen(film, "trailer")}
              />
              <p className="card-tag">{film.year} · Film</p>
              <h3>{film.title}</h3>
              <p className="byline">Directed by<br />{film.director}</p>
              <button onClick={() => onOpen(film, "program")}>
                View programme <span>→</span>
              </button>
            </article>
          ))}
        </div>
...
function About() {
  return (
    <section id="about" className="about-section">
      <div className="section-decor about-decor" aria-hidden="true">
        <img
          className="decor-angel"
          src="/assets/angel-cloud-exlibris.png"
          alt=""
        />
        <img className="decor-cloud" src="/assets/cloud.png" alt="" />
      </div>
...
function Footer() {
  return (
    <footer>
      <img src="/assets/floral-background.png" alt="" />
...
export default function App() {
  ...
  const openArt = (art, index) => {
    setArtIndex(index);
    setModal({ ...art, kind: "art" });
  };
  useEffect(() => {
    if (modal?.kind === "art") setModal({ ...artworks[artIndex], kind: "art" });
  }, [artIndex]);
  return (
    <>
      <div className={`loader ${loading ? "" : "gone"}`} aria-hidden="true">
        <p>Tapestry</p>
      </div>
